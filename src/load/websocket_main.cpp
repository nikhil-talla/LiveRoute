...
[[nodiscard]] bool parse_arguments(
    int argc, char** argv, std::string* target, std::string* token_file,
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (argument == "--profile") {
      const auto parsed = liveroute::load::parse_workload_profile(value);
  ...
  if (*self_check) {
    ...
    configuration->active_trips = 2;
    configuration->event_count = 4;
    configuration->events_per_second = 100000;
    configuration->location_burst_size = 2;
    configuration->suffix_size = 2;
    configuration->deadline = 5s;
  }
  return !target->empty() && !token_file->empty() && configuration->is_valid();
}

void usage() {
  std::cerr
      << "usage: liveroute_websocket_loadgen --target ws://HOST:PORT/ws"
         " --token-file PATH [--profile NAME] [--seed N] [--trips N]"
         " [--events N] [--events-per-second N] [--burst-size N]"
         " [--suffix-size N] [--reservation-percent N] [--deadline-ms N]"
         " [--self-check]\n";
}
...
int main(int argc, char** argv) {
  ...
  if (!parse_arguments(argc, argv, &target, &token_file, &configuration,
                       &self_check)) {
    usage();
    return 2;
  }
  const auto result = liveroute::load::run_websocket_workload(
      target, token_file, configuration);
  liveroute::load::write_websocket_load_report(
