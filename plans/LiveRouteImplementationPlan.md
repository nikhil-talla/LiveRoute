...
- Backend GPS handling is transport-only: assign epoch-scoped observation sequences and retain only the newest sample per type while disconnected/overloaded, with explicit coalesced/dropped status. The backend never infers domain boundaries.
- On the owner shard, apply every admitted non-stale ordinary observation and advance planner-state freshness. When no explicit feasibility-changing replan is active, acknowledge with `replan_scheduled = false` and start no provider/planner work.
- If an explicit high/critical replan is active, a newer ordinary observation cancels the obsolete attempt and refreshes at most one bounded replacement seed while preserving the highest-priority explicit trigger. A boundary that must never be lost is an explicit non-coalescible domain event rather than an intermediate GPS sample.
- V1 does not define current-route geometry, configurable geofences, raw-coordinate boundary inference, or an ETA/progress-only response. Signed slack is computed for proposal notification/result metadata after an explicit trigger; it is not a raw-GPS admission trigger.
