...
For a scheduled child, `arrival_unix_ms` is the parent's last scheduled end plus the immutable matrix travel duration, or `current_time` plus current-location travel for the first suffix activity. Unreachable or checked-overflowing travel produces no scheduled alternative. Candidate intervals are then generated by these exact rules:
...
4. At each emitted start, let `minimum_positive_seconds = max(1, min_duration_seconds)`. Let `finish_cap_seconds` be the nonnegative whole-second floor of the smallest remaining span to the containing window close, planning-horizon end, and mandatory deadline when present, additionally capped by `max_duration_seconds`. If `finish_cap_seconds < minimum_positive_seconds`, emit no duration. If `can_shorten = false`, emit only `preferred_duration_seconds`, and only when it is positive and no greater than `finish_cap_seconds`. If `can_shorten = true`, first emit `min(max(1, preferred_duration_seconds), finish_cap_seconds)` when it is at least `minimum_positive_seconds`, then emit `minimum_positive_seconds` when different. These are the only generated shortened durations.
5. Independently, a movable non-fixed activity's exact current-plan interval is included when it passes every hard constraint, even when its duration is greater than the preferred duration. This preserves an unchanged user interval as an alternative. Exact duplicate `(activity_ordinal, start_unix_ms, end_unix_ms)` alternatives produced by the window, duration, or baseline rules are removed.

Within one activity, scheduled alternatives are considered by ascending start time; at one start, the exact unchanged current-plan interval comes first, followed by generated durations in descending duration order. The skip child comes after all scheduled alternatives. Thus each normalized window contributes at most two start boundaries and two generated durations, plus at most one exact baseline interval per activity and one skip. An intermediate duration between the longest generated duration and the minimum is intentionally outside the V1 proposal set: the minimum preserves every downstream feasibility benefit of shortening, while the complete-candidate objective still prefers the longer generated choice whenever both lead to feasible completions. Adding time-grid samples, downstream-derived intermediate durations, or another start boundary is a new planner-policy version, not an implementation detail.
...
- candidate-generation tests prove that starts are limited to the exact current interval, per-window earliest-arrival boundary, and legal current-plan-start boundary; durations are limited to the exact baseline, longest generated preferred/capped value, and positive minimum rules; duplicates are removed; fixed/immovable omission is preserved; skip is last; and no minute-grid or unlisted intermediate-duration alternative appears;
