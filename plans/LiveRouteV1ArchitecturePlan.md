...
V1 proposal selection uses the fixed `liveroute-v1-lexicographic-1` objective in `plans/LiveRouteV1ContractSpec.md`, not tunable scalar weights. Hard feasibility is enforced before comparison; remaining candidates lexicographically preserve higher-priority activities, maximize user utility, minimize lateness/compression/travel/current-plan disruption, and use a canonical schedule key for complete and beam-retention ties. This policy affects suggestions only and never changes the user-authoritative current plan without explicit acceptance.

The V1 beam branches on remaining-activity order and allowed omission, not arbitrary timestamps. Schedule alternatives use only the contract's finite boundary set: an exact legal current-plan interval, each window's earliest reachable legal start, and a legal current-plan start, with at most the longest preferred/capped whole-second duration and positive minimum duration at a generated start. The effective suffix start is the later of current time and every scheduled preserved-prefix end, so an unchanged started activity cannot overlap revised work. Parent, activity, start, duration, and skip order and both candidate budgets are normative in `plans/LiveRouteV1ContractSpec.md`; there is no continuous-time search or minute-grid sampling in V1.
