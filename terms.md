...
- **V1 planner objective:** The fixed `liveroute-v1-lexicographic-1` hard-feasibility and candidate-ordering policy. It uses no scalar weights: higher-priority preservation precedes utility, lateness, compression, travel, current-plan disruption, and a canonical final tie-break.
- **V1 candidate-generation set:** The finite schedule alternatives searched by that objective. The beam branches on activity order and allowed omission; starts come only from a legal exact current interval, an open window's earliest reachable legal boundary, or a legal current-plan-start boundary, and a generated start has at most a preferred/capped whole-second duration and a positive minimum duration. Exact order and budget counting are normative in `plans/LiveRouteV1ContractSpec.md`; V1 has no continuous timestamp search.
