...
- **Benchmark raw artifact:** One schema-validated `liveroute.benchmark.v1` JSON document containing exact run dimensions, measured operations/time, counters, fixed-bucket histograms, and gauges. Console text and already-derived percentile tables are not raw aggregation input.
- **Benchmark aggregate artifact:** A `liveroute.benchmark.aggregate.v1` JSON report that partitions compatible raw runs by exact dimensions, merges their primitive counts/buckets, and only then derives throughput and p50/p95/p99.
- **Planner allocation scope:** The thread-local benchmark measurement boundary from `assemble_beam_search_input` through complete stored-proposal/no-proposal construction. It counts allocation calls and requested bytes on that planner worker while excluding provider, transport, persistence, logging, serialization, and artifact-generation work.
- **Optimization evidence ledger:** `plans/summaries/optimization-evidence-ledger.md`, which records exact artifact identities, measurements, correctness checks, and whether an experiment was accepted, rejected, or reverted. `docs/optimizations.md` is the reader-facing summary. Illustrative planning numbers are not evidence.
- **Planner activity columns:** The private, worker-owned Phase 19 structure-of-arrays view derived once from a valid `BeamSearchInput`. It accelerates hot-loop reads without changing domain, planner input/output, candidate-order, wire, or storage contracts.
- **Phase 19 native timing artifact:** A `planner-layout-timing-v1` benchmark artifact produced without profiler instrumentation and used for planner throughput/p50/p95/p99 and allocation guards.
- **Phase 19 profile artifact:** A `planner-layout-profile-v1` benchmark artifact derived from the checksum-pinned Callgrind run with fixed simulated cache geometry. It reports instructions, data/cache references and misses, and branch/misprediction counts; its instrumented wall time is not native-latency evidence.
- **E5 coordinate cell:** The deterministic route-cache coordinate obtained by multiplying latitude or longitude by 100,000 and rounding to the nearest signed integer with halves away from zero. Missing routes are queried at the corresponding grid point so cache values do not depend on the first requester.
- **Fresh route-cache entry:** A cached provider estimate no more than six hours old. A hit does not extend its age.
- **Stale route-cache fallback:** A completely covered matrix assembled from expired entries no more than 24 hours old after a `PROVIDER_UNAVAILABLE` fetch. It is successful only with `routing_quality = STALE_CACHE`; other failure statuses cannot use it.
...
- **Plan proposal message:** A separately persisted structured planner result containing the proposal identity/base current plan, preserved prefix, revised suffix, and changes such as moved, added-back, or skipped activities. It is advisory rather than the current plan; V1 suggestions never shorten activities.
...
- **Expected trip revision:** An optimistic-concurrency check on a durable trip edit. It means, "apply this command only if the trip is still at revision X." If the client read revision 5 but another edit already advanced the trip to revision 6, a command with `expected_trip_revision = 5` is rejected as stale instead of overwriting the newer edit. This is different from `planner_state_version` (C++ runtime state), `runtime_epoch` (backend lease generation), and `mutation_sequence` (durable command ordering).
...
- **User-authored open windows:** The authoritative normalized UTC availability intervals entered for a V1 activity, validated and persisted by the backend, then mirrored to C++. Normal V1 replanning uses these windows directly and performs no place-hours request.
- **Deserialization timing:** Application-controlled transport admission time. WebSocket measures delivered text bytes through strict JSON/schema validation; gRPC measures callback entry with an already decoded generated message through validated Protobuf-to-domain construction. Framework framing, HTTP/2, and hidden codec work are excluded.
- **Seeded-hours readiness-validation domain:** Every recurring opening date representable in a valid V1 local date range—`1970-01-01` through `9999-12-30`—plus next-day closing endpoints. Validation is eager and transition-directed over pinned TZif gap/fold transitions; it is not a rolling or lazy date-expansion horizon.
- **V1 planner objective:** The fixed `liveroute-v1-lexicographic-1` hard-feasibility and candidate-ordering policy. It uses no scalar weights: preserving higher-priority activities and avoiding skips precede utility, lateness, travel, current-plan disruption, and a canonical final tie-break.
- **V1 candidate-generation set:** The finite schedule alternatives searched by that objective. The beam branches on activity order and allowed omission; starts come only from a legal exact current interval, an open window's earliest reachable legal boundary, or a legal current-plan-start boundary. A scheduled activity keeps its exact baseline duration, and an omitted activity added back uses its preferred positive duration. Exact order and budget counting are normative in `plans/LiveRouteV1ContractSpec.md`; V1 has no continuous timestamp or variable-duration search.
- **Changed activity count:** A late suggestion-ranking tie-breaker that counts each remaining activity at most once when its state, interval, or position among activities scheduled in both the authoritative and proposed suffix differs. It does not reject a proposal or mutate the user's plan.
- **Causal plan reason:** A sorted, deduplicated reason derived from the triggering typed event or immutable matrix-based lateness/reservation-risk facts. Only proposal segments that actually differ from the authoritative baseline inherit causal reasons.
- **Outcome plan reason:** `DEADLINE_BUDGET` or `NO_FEASIBLE_PLAN` added from search termination. It explains result quality/absence and is never copied to proposal segments.
- **Planner expansion:** One actual finite-generator invocation for a partial candidate and one undecided activity. It may emit no children. Invalid routes/times/durations discarded before emission are neither materialized nor separately counted.
